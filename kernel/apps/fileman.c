...
static void fm_navigate_to(uint32_t new_dir, LstID lst,
    ...
    if (!fs_acl_check(new_dir, ACL_READ)) {
            ...
            if (line) {
                ...
                if (fm_strcmp(name, "(dossier") != 0) {
                    if (is_dir) {
                        ...
                        uint32_t saved = g_cwd; g_cwd = g_fm.dir_idx;
        ...
        if (app_button_touched(btn_new_fil)) {
            if (!session_has_permission(PERM_FS_WRITE)) {
                ...
                if (fm_dialog_input("Nouveau fichier", "Nom du fichier :", fname, FM_NAME_MAX)) {
                    uint32_t sv = g_cwd; g_cwd = g_fm.dir_idx;
                    if (fs_find(fname) < 0) fs_write_file(fname, (const uint8_t*)"", 0);
                    if (session_has_permission(PERM_FS_READ)) app_fileeditor_run(fname);
                    g_cwd = sv;
                    fm_populate_list(lst, g_fm.dir_idx);
                    app_set_label_text(lbl_status, "");
                }
                ...
                if (line) {
                    ...
                    if (fm_strcmp(name, "(dossier") != 0) {
                        if (isd) {
                            ...
                            // Forcer TextEdit même pour .tex (éditer, pas exécuter)
                            if (!session_has_permission(PERM_FS_READ)) {
                                app_set_label_text(lbl_status, "Permission refusee."); app_set_label_color(lbl_status, FM_STATUS_ERR);
                            } else {
                                uint32_t sv = g_cwd; g_cwd = g_fm.dir_idx;
