...
//   - cmd_sl()           : easter egg
//
// Note : le terminal et textedit sont des apps autonomes (apps/).
// shell.c ne les inclut plus — il appelle leurs points d'entrée
// via g_print_hook uniquement (hook installé par chaque app).
...
void dispatch_ray64(const char* cmd, const char* args) {
    if (strcmp(cmd, "ls") == 0) {
        ...
        if (!session_has_permission(PERM_FS_WRITE)) { print_string("Permission refusee\n"); return; }
        char cmod_name[64], cmod_perms[32];
        int sp = -1;
        ...
        if      (strcmp(cmod_perms, "private") == 0) new_perms = ACL_OWNER_FULL;
        else if (strcmp(cmod_perms, "shared")  == 0) new_perms = ACL_OWNER_FULL | ACL_ADMIN_FULL;
        else if (strcmp(cmod_perms, "public")  == 0) new_perms = ACL_OWNER_FULL | ACL_ADMIN_FULL | ACL_OTHER_R;
        else if (strcmp(cmod_perms, "open")    == 0) new_perms = ACL_OWNER_FULL | ACL_ADMIN_FULL | ACL_OTHER_R | ACL_OTHER_W | ACL_OTHER_X;
        else {
            uint32_t val = 0;
            for (int ii = 0; cmod_perms[ii] != '\0'; ii++) {
                if (cmod_perms[ii] >= '0' && cmod_perms[ii] <= '9') val = val * 10 + (uint32_t)(cmod_perms[ii] - '0');
                else { print_string("ray64.chmod : valeur invalide\n"); return; }
            }
        ...
        char co_name[64], co_owner[SESSION_NAME_LEN];
        int sp = -1;
        for (int ii = 0; args[ii] != '\0'; ii++) {
            if (args[ii] == ' ') { sp = ii; break; }
        }
        if (sp < 0) { print_string("ray64.chown : usage : ray64.chown <nom> <session>\n"); return; }
        int nlen = sp < 63 ? sp : 63;
...
void dispatch_sys(const char* cmd, const char* args) {
    ...
    if (strcmp(cmd, "help") == 0) {
        ...
        print_string("  ray64.acl   <nom>     Voir les permissions\n");
        ...
        print_string("V1.6 :\n");
        print_string(" -Apps autonomes avec signature TEX (apps/app.h)\n");
        print_string(" -Terminal migre en app independante (apps/terminal.c)\n");
        print_string(" -Scanner memoire kernel pour decouverte automatique des apps\n");
        print_string("V1.5 :\n");
...
void dispatch_tex(const char* cmd, const char* args) {
    if (strcmp(cmd, "run") == 0) {
        ...
        for (uint32_t i = 0; i < dir->child_count; i++) {
            ...
            int nlen = 0;
            while (n[nlen] != '\0') nlen++;
            if (nlen > 4 && n[nlen-4] == '.' && n[nlen-3] == 't'
                         && n[nlen-2] == 'e' && n[nlen-1] == 'x') {
                print_string("  ");
